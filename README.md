...
The depth coordinate $`z`$ is scaled with characteristic length $`z_0`$, time <img src="http://www.sciweavers.org/tex2img.php?eq=t%27&bc=White&fc=Black&im=jpg&fs=12&ff=arev&edit=0" align="center" border="0" alt="t'" width="15" height="17" /> is scaled with characteristic time $`z_0/f_c`$ and infiltration rate $`I(t)`$ (or volumetric flux) is scaled with the infiltration capacity $`f_c`$. Therefore, the dimensionless variables are $`z'=z/z_0`$, $`t'=tf_c/z_0`$, and $`f'=I(t)/f_c`$.
