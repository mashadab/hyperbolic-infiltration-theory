#Infiltration in an initially-dry soil with exponential porosity decay
...
#parameters
simulation_name = f'exponential_analytical'
...
R = 0.64   #dimensionless rainfall rates

#spatial discretization
zbottom = 1
Nz   = 1000
zc   = np.linspace(0,zbottom,Nz) #depth array
phi  = np.transpose([phi_0*np.exp(-zc/z0)]) #porosity vector: upper layer
...
t_interest = [0,0.05,0.09652168126992737 +0.0005,0.15,0.2471995873155689,tmax] 
...
theta_0 = opt.fsolve(func_R2phil_U,0.01)[0]+0.00001   
...
print("############################################################# \n")
print("7 (top) Volume fraction with depth")
print("############################################################# \n")

# First set up the figure, the axis
fig,([ax1,ax2,ax3,ax4,ax5,ax6]) = time_sequenced_figure(zbottom,0,0,1,t_interest)

xf = n/(m - n)*z0*np.log(1+(m-n)/(n*fcbyR*phi_0*z0*(1 - s_wr - s_gr))*fcbyR**(1/n)*t_interest[0])
S_w_analy_int = s_wr*np.ones((Nz,1))
S_w_analy_int[zc<=xf] = np.transpose([s_wr + (1 - s_gr - s_wr)*(1/fcbyR)**(1/n)*np.exp(m*zc[zc<=xf]/(n*z0))])
...
xf = n/(m - n)*z0*np.log(1+(m-n)/(n*fcbyR*phi_0*z0*(1 - s_wr - s_gr))*fcbyR**(1/n)*t_interest[1])
S_w_analy_int = s_wr*np.ones((Nz,1))
S_w_analy_int[zc<=xf] = np.transpose([s_wr + (1 - s_gr - s_wr)*(1/fcbyR)**(1/n)*np.exp(m*zc[zc<=xf]/(n*z0))])
...
res = solve_ivp(rhs_stage2, (0, t_interest[2] - ts), [-1e-15+zs[0],1e-15+zs[0]],t_eval=[t_interest[2]-ts])

S_w_analy_int = (1-s_gr)*np.ones((Nz,1))
S_w_analy_int[zc<=res.y[0,0]] = np.transpose([s_wr + (1 - s_gr - s_wr)*(1/fcbyR)**(1/n)*np.exp(m*zc[zc<=res.y[0,0]]/(n*z0))])
...
res = solve_ivp(rhs_stage2, (0, t_interest[3] - ts), [-1e-15+zs[0],1e-15+zs[0]],t_eval=[t_interest[3]-ts])
S_w_analy_int = (1-s_gr)*np.ones((Nz,1))
S_w_analy_int[zc<=res.y[0,0]] = np.transpose([s_wr + (1 - s_gr - s_wr)*(1/fcbyR)**(1/n)*np.exp(m*zc[zc<=res.y[0,0]]/(n*z0))])
S_w_analy_int[zc>=res.y[1,0]] = s_wr
...
res = solve_ivp(rhs_stage2, (0, tnew[-1]), [-1e-15+zs[0],1e-15+zs[0]],t_eval=[t_interest[4]-ts])
...
plt.savefig(f"{simulation_name}_CL{theta_0}.pdf")

print("############################################################# \n")
print("7 (bottom) Saturation with depth")
print("############################################################# \n")
...
plt.savefig(f"swvsZpanelshock_{simulation_name}_phi0{theta_0}.pdf")

print("############################################################# \n")
print("8 Dimensionless infiltration rate with dimensionless time")
print("############################################################# \n")
